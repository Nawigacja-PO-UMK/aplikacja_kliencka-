...
public class GeocoderNominatim {
     ...
     */
    protected com.search_location.Address buildAndroidAddress(JsonObject jResult) throws JsonSyntaxException{
        com.search_location.Address gAddress = new com.search_location.Address(mLocale);
        ...
        if(jResult.has("extratags"))
        {
            JsonObject extratags=  jResult.get("extratags").getAsJsonObject();
            if(extratags.has("website"))
            {
                gAddress.setUrl(extratags.get("website").getAsString());
            }
            if(extratags.has("opening_hours"))
            {
                gAddress.setOpen_closs(extratags.get("opening_hours").getAsString());
            }

            if(extratags.has("contact:facebook"))
            {
                gAddress.setFacebook(extratags.get("contact:facebook").getAsString());
            }
            else
            if(extratags.has("facebook"))
            {
                gAddress.setFacebook(extratags.get("facebook").getAsString());
            }

            if(extratags.has("contact:phone"))
            {
                gAddress.setPhone(extratags.get("contact:phone").getAsString());
            }
            else
                if(extratags.has("phone"))
                {
                    gAddress.setPhone(extratags.get("phone").getAsString());
                }
        }
